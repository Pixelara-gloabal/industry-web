import { Award, CheckCircle2, ClipboardCheck, FileCheck2, SearchCheck, ShieldCheck } from "lucide-react";
...
const qualityPillars = [
  ...
  },
  {
    ...
    copy: "Castings and forged alloy shafts undergo ultrasonic non-destructive testing (NDT), magnetic particle crack detection, and Rockwell hardness testing post carburization.",
  },
  {
    icon: ShieldCheck,
    title: "Full Load Bench Testing",
    copy: "100% of assembled speed reducers undergo no-load and full-load spin testing to monitor noise levels (dB), oil seal thermal rise, vibration velocity (mm/s), and backstop engagement.",
  },
  {
    icon: FileCheck2,
    title: "Material Traceability & Certs",
    copy: "Full EN 10204 3.1 material test certificates, heat treatment charts, and dynamic balancing reports (ISO 1940 Grade G6.3) are supplied with every major drive unit shipment.",
  },
];
      ...
      <main className="mx-auto max-w-[1440px] px-5 py-20 sm:px-8 lg:px-12">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {qualityPillars.map(({ icon: Icon, title, copy }, index) => (
            <article
              key={title}
              className="flex flex-col justify-between rounded-xl border border-white/10 bg-[#15191c] p-8 shadow-2xl"
            >
              <div>
        ...
        <section className="mt-16 rounded-xl border border-[#f3a329]/40 bg-[#15191c] p-8 sm:p-12 shadow-2xl">
          <div className="grid gap-8 lg:grid-cols-[1fr_1.2fr] items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="rounded border border-white/10 bg-[#0b0d0e] p-5">
              ...
              <div className="rounded border border-white/10 bg-[#0b0d0e] p-5">
                <div className="flex items-center gap-2 text-[#f3a329] font-bold text-sm">
                  <CheckCircle2 className="size-4" /> CE Compliance
                </div>
                <p className="mt-2 text-xs text-[#a7adb3]">
              ...
              <div className="rounded border border-white/10 bg-[#0b0d0e] p-5">
                <div className="flex items-center gap-2 text-[#f3a329] font-bold text-sm">
                  <CheckCircle2 className="size-4" /> ATEX Zone 22
                </div>
                <p className="mt-2 text-xs text-[#a7adb3]">
              ...
              <div className="rounded border border-white/10 bg-[#0b0d0e] p-5">
                <div className="flex items-center gap-2 text-[#f3a329] font-bold text-sm">
                  <CheckCircle2 className="size-4" /> AGMA &amp; ISO 1328
                </div>
                <p className="mt-2 text-xs text-[#a7adb3]">
                  Gear geometry, load rating, and tooth surface contact stress calculations per AGMA 2001.
                </p>
              </div>
            </div>
          </div>
